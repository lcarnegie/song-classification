...
Recently, AI-powered tools like Suno [@suno] have emerged, targeting consumers interested in "creating" their own music. This raises questions about how machines understand music, especially in the case of generative models such as Suno's. While more research-based proofs of concept like Meta's MuseGen [@musegen] have existed for years, tools like Suno adopt a more consumer-focused approach. But how does a word entered into a prompt box translate into the auditory representation humans understand as music? Do different artificial intelligence systems have different conceptualizations of commonly accepted human concepts of genre?

To understand if there is a different between conceptualizations of genre between different machine learning systems, we train several different machine learning algorithms in the task of predicting genre, based on the dataset of the timbral audio features of songs across 10 genres extract from the Free Music Archive [@fma]. Then, we ask the 'small' variant of Meta MuseGen to create 'quintessential' representations of different genres of music using systematic prompts, and test if the algorithm can accurately predict the genre based on extracted audio features.
